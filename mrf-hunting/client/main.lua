...
AddEventHandler('mrf-hunting:client:slaughterAnimal', function(entity)
    ...
    CoreName.Functions.Progressbar("harv_anim", "Slaughtering Animal", Config.SlaughteringSpeed, false,
        ...
        if AnimalLootMultiplier:read(entity) ~= nil and AnimalLootMultiplier:read(entity) ~= false then
            TriggerServerEvent('mrf-hunting:server:AddItem', animal, NetworkGetNetworkIdFromEntity(entity),
                AnimalLootMultiplier:read(entity))
        else
            -- defalut values for multipiler
            TriggerServerEvent('mrf-hunting:server:AddItem', animal, NetworkGetNetworkIdFromEntity(entity), 'default')
        end
    end)
end)

AddEventHandler('mrf-hunting:client:sellREQ', function()
    TriggerServerEvent('mrf-hunting:server:sellmeat')
end)

RegisterNetEvent('mrf-hunting:client:ForceRemoveAnimalEntity')
AddEventHandler('mrf-hunting:client:ForceRemoveAnimalEntity', function(entity)
    DeleteEntity(entity)
    ...
    if outPosition.x ~= 0 and outPosition.y ~= 0 and outPosition.z ~= 0 then
        Citizen.CreateThread(function()
            ...
            if startSpawningTimer == 0 then
                ...
                TriggerServerEvent('mrf-hunting:server:choiceWhichAnimalToSpawn', coord, outPosition, zone_type,
                    indicator)
            else
...
RegisterNetEvent('mrf-hunting:client:useBait', function()
...
RegisterNetEvent('mrf-hunting:client:spawnAnimal', function(coord, outPosition, C_animal, was_llegal, indicator)
    ...
    TriggerServerEvent('mrf-hunting:server:removeBaitFromPlayerInventory')
...
RegisterNetEvent('mrf-hunting:client:spawnanim', function(model, was_llegal)
...
RegisterNetEvent('mrf-hunting:client:clearTask')
AddEventHandler('mrf-hunting:client:clearTask', function()
    local playerPed = PlayerPedId()
