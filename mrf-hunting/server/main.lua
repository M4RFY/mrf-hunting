...
RegisterServerEvent("mrf-hunting:server:AddItem", function(data, netId, multiplier)
    local src = source
...
RegisterServerEvent('mrf-hunting:server:sellmeat', function()
    local src = source
    local Player = CoreName.Functions.GetPlayer(src)
    local price = 0
...
CoreName.Functions.CreateUseableItem("huntingbait", function(source, item)
    TriggerClientEvent('mrf-hunting:client:useBait', source)
end)

RegisterServerEvent('mrf-hunting:server:removeBaitFromPlayerInventory')
AddEventHandler('mrf-hunting:server:removeBaitFromPlayerInventory', function()
    local Player = CoreName.Functions.GetPlayer(source)
    Player.Functions.RemoveItem("huntingbait", 1)
end)

RegisterServerEvent('mrf-hunting:server:choiceWhichAnimalToSpawn',
    function(coord, outPosition, is_llegal, indicator)
        ...
        TriggerClientEvent('mrf-hunting:client:spawnAnimal', src, coord, outPosition, C_animal, is_llegal, indicator)
    end)
...
CoreName.Commands.Add("spawnanimal", "Spawn Animals (Admin Only)",
    { { "model", "Animal Model" }, { "was_llegal", "area of hunt true/false" } }, false, function(source, args)
        TriggerClientEvent('mrf-hunting:client:spawnanim', source, args[1], args[2])
    end, 'admin')

CoreName.Commands.Add("clearTask", "Clear Animations", {}, false, function(source)
    TriggerClientEvent('mrf-hunting:client:clearTask', source)
end, 'user')
